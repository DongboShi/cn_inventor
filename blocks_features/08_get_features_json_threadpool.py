        ...
        for i in range(len(patents_data['patent_title'])):
            for j in range(i + 1, len(patents_data['patent_title'])):
                if i != j:
                    ...
                    # app_i
                    app_i_value = lst_intersec(literal_eval(patents_data['applicants_lst'][i]),literal_eval(patents_data['applicants_lst'][j]))
                    features_data['app_i'].append(app_i_value)
                    features_data['app_i_est'].append(zeroifs(literal_eval(patents_data['applicants_lst'][i]),literal_eval(patents_data['applicants_lst'][j])))
                    features_data['app_i_per'].append(max(lst_per(app_i_value, len(literal_eval(patents_data['applicants_lst'][i]))), lst_per(app_i_value, len(literal_eval(patents_data['applicants_lst'][j])))))
                    # app_s
                    features_data['app_s'].append(match_name_lr(patents_data['app_first'][i], patents_data['app_first'][j]))
                    features_data['app_s_est'].append(zeroifs(patents_data['app_first'][i], patents_data['app_first'][j]))
                    # inventor_i
                    ...
                    # ipc_c
                    ipc_c_value = lst_intersec(literal_eval(patents_data['ipc_class'][i]), literal_eval(patents_data['ipc_class'][j]))
                    features_data['ipc_c'].append(ipc_c_value)
                    features_data['ipc_c_est'].append(zeroifs(literal_eval(patents_data['ipc_class'][i]), literal_eval(patents_data['ipc_class'][j])))
                    features_data['ipc_c_per'].append(max(lst_per(ipc_c_value, len(literal_eval(patents_data['ipc_class'][i]))),
                                              lst_per(ipc_c_value, len(literal_eval(patents_data['ipc_class'][j])))))

                    # ipc_g
                    ipc_g_value = lst_intersec(literal_eval(patents_data['ipc_group'][i]), literal_eval(patents_data['ipc_group'][j]))
                    features_data['ipc_g'].append(ipc_g_value)
                    features_data['ipc_g_est'].append(zeroifs(literal_eval(patents_data['ipc_group'][i]), literal_eval(patents_data['ipc_group'][j])))
                    features_data['ipc_g_per'].append(max(lst_per(ipc_g_value, len(literal_eval(patents_data['ipc_group'][i]))),
                                              lst_per(ipc_g_value, len(literal_eval(patents_data['ipc_group'][j])))))

                    # title
                    features_data['title'].append(match_name_lr(patents_data['patent_title'][i], patents_data['patent_title'][j]))
                    features_data['title_est'].append(zeroifs(patents_data['patent_title'][i], patents_data['patent_title'][j]))
                    # keywords
                    ...
                    # geo
                    geo_value = calculate_geo_score(i, j, patents_data['country'], patents_data['province'], patents_data['city'], patents_data['district'], patents_data['road'], patents_data['latitude'], patents_data['longitude'])
                    features_data['geo'].append(geo_value)
                    features_data['geo_est'].append(geo_exist(i, j, geo_value, patents_data['country'], patents_data['province'], patents_data['city'], patents_data['district'], patents_data['road'], patents_data['latitude'], patents_data['longitude']))
                    # citing
                    features_data['citing_rp'].append(citing_relationship(i, j, patents_data['citing_lst'], patents_data['cited_lst'], patents_data['ida']))
                    features_data['citing_lst_est'].append(citing_rp_est(i, j, patents_data['citing_lst'], patents_data['cited_lst'])[0])
                    features_data['cited_lst_est'].append(citing_rp_est(i, j, patents_data['citing_lst'], patents_data['cited_lst'])[1])
                    # diff_days
                    ...
                    # address_s
                    features_data['address_s'].append(match_name_lr(patents_data['detailed_address'][i], patents_data['detailed_address'][j]))
                    features_data['address_s_est'].append(zeroifs(patents_data['detailed_address'][i], patents_data['detailed_address'][j]))
